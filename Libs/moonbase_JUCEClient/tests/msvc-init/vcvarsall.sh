...
function find_vcvarsall() {
  ...
  VSINSTALLDIR=$("${VSWHERE_PATH}" -latest -property installationPath)

  if [[ -z "${VSINSTALLDIR}" ]]; then
      echo "Visual Studio install not found at ${VSWHERE_PATH}"
  ...
  if [[ -n "${VSINSTALLDIR:-}" ]]; then
    ...
    if [[ -n "${VSWHEREPATH:-}" ]]; then
      ...
      vswhere=$(command -v 'vswhere' 2>/dev/null || '/c/Program Files (x86)/Microsoft Visual Studio/Installer/vswhere.exe')
    fi
